...
Page({
  ...
  clickButton: function (event) {
    if (this.data.result == 0) {
      ...
      this.setData({
        any:this.data.result.split('+')
      })
      this.setData({
        
        result: this.calculate(this.data.result),
        number: this.data.result.split('+'),
        per:this.calculate(this.data.result)
      ...
      this.setData({
        ...
        var arr1= new Array();
        arr1=this.data.number;
        for(var i=0;i<arr1.length;i++){
          arr1[i]=arr1[i]/this.data.per;
          arr1[i]=100*arr1[i];
          arr1[i]=arr1[i].toFixed(2)
          arr1[i]=arr1[i]+"%"
        }
